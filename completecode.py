...
def root4_fun(X, y , title , xlabel , ylabel):
    ...
    root4.geometry('1050x550+200+100')
    ...
    alpha = tk.Spinbox(frame3, from_=1, to=15, width=20, bd=4 , background="#b3e6ff")
    alpha.grid(column=1, row=2, padx=8, pady=4, sticky=tk.N)
    ...
    l1_ratio = tk.Spinbox(frame4, from_=0.0, to=1.0, width=20, bd=4 , background="#b3e6ff")
    l1_ratio.grid(column=1, row=0, padx=8, pady=4)
    ...
    X_s, y_s = preprocessing.ret_X_y_s()
    sc_x, sc_y = preprocessing.ret_scaler()
    # Regression models class
    reg_obj = Regressors(X_s, y_s)

    def _apply():
        flag = 1
        # -----------------------------------MLCode is here
        if reg_type.get() == 'LinearRegression':
            regressor = reg_obj.lin_reg()
        elif reg_type.get() == 'PolynomialRegression':
            flag = 0
            n = int(features.get())
            if regulari_type.get() == 'None':
                regressor = reg_obj.poly_reg(n)
            elif regulari_type.get() == 'Ridge':
                alp = float(alpha.get())
                regressor = reg_obj.ridge(alp, n)
            elif regulari_type.get() == 'Lasso':
                apl = float(alpha.get())
                regressor = reg_obj.lasso(apl, n)
            elif regulari_type.get() == 'ElasticNet':
                ratio = float(l1_ratio.get())
                apl = float(alpha.get())
                regressor = reg_obj.elastic(apl, ratio, n)
        ...
        fig.add_subplot(111).plot(sc_x.inverse_transform(X_grid),
                                  sc_y.inverse_transform(regressor.predict(X_fit)))
